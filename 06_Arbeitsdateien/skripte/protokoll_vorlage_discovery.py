...
OPEN_MARK = RGBColor(0xB0, 0x4A, 0x00)   # Hinweisfarbe für unbeantwortete Punkte

FOOTER = ("CertoClav Sterilizer GmbH  ·  Leonding, Österreich  ·  "
          "support@certoclav.com  ·  www.certoclav.com")

OFFEN = "__OFFEN__"   # Marker: Frage im Termin nicht beantwortet
...
def build(path, termin=None):
    ...
    if d and d.get("laborprozess"):
        ...
        for titel, key in [("Analytik", "analytik"), ("Bebrütung & Entsorgung", "bebruetung"),
                           ("Mengengerüst", "mengen")]:
            p = doc.add_paragraph()
            p.paragraph_format.space_before = Pt(8)
            p.paragraph_format.space_after = Pt(0)
            p.add_run(titel).font.bold = True
            bullets(doc, lp[key])
